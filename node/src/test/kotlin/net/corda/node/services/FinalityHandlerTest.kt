...
class FinalityHandlerTest {
    ...
    fun `sent to flow hospital on error and attempted retry on node restart`() {
        ...
        bob.assertFlowSentForObservationWithConstraintError(finalityHandlerId)
    ...
    fun `disabled if there are no old CorDapps loaded`() {
        ...
        val error = bob.assertFlowSentForObservationWithError(finalityHandlerId)
        assertThat(error).hasMessageContaining("This node does not support the old unsafe usage of FinalityFlow")
        assertThat(bob.getTransaction(stx.id)).isNull()
    }
    ...
    private fun TestStartedNode.assertFlowSentForObservationWithError(runId: StateMachineRunId): Throwable {
        val keptInForObservation = smm
                .flowHospital
                .track()
        ...
        return keptInForObservation.errors.single()
    }

    private fun TestStartedNode.assertFlowSentForObservationWithConstraintError(runId: StateMachineRunId) {
        val error = assertFlowSentForObservationWithError(runId)
